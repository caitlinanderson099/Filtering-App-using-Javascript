...
// Filter Results - return an array of filtered properties (results)
function filterProperties() {
    const filteredProperties = properties.filter(property => {
        // Check if location matches
        const propertyLocation = property.location.toLowerCase();
        const filterValue = locationFilter.value.toLowerCase();
        // Check if it doesn't match, returns false and doesn't add to the array
        ...
        const priceValue = parseFloat(property.price.replace(/\$/g, '').replace(/,/g, '')) // removes $ and , from price and makes the number a FLOATING number
...
function populateResults(filteredResults) {
    ...
    // if statement to check if the filtered properties are empty
    if (filteredResults.length === 0) {
        resultsDiv.innerHTML = `
            <p class="no-results"> No Results Found </p>
        `
    } else {
        // this is what happens if filteredResults is NOT empty
